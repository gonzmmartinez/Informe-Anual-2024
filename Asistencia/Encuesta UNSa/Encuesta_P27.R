...
Data <- data.frame(Tipo = unlist(strsplit(paste0(Raw$P27, collapse=", "), ", "))) %>%
  ...
  ungroup() %>%
  mutate(Tipo = ifelse(Cantidad < 3, "Otras medidas", Tipo)) %>%
  group_by(Tipo) %>%
...
# Colores
Colores <- colorRampPalette(c("lightgray", "#f2904c"))(3)

...
grafico <- ggplot(Data, aes(x=Año, y=Cantidad)) +
  ...
  annotate(geom="rect", xmin=4.55, xmax=5.45, ymin=(Data %>% filter(Año == 2024))$Cantidad, ymax=Estimacion,
           linetype=2, color="gray", fill="gray", alpha=0.5) +
  geom_text(aes(y=Cantidad, label=formatC(Cantidad, big.mark = ".", decimal.mark = ",")), family="font", fontface="bold", color="white", size=8, vjust=2) +
  annotate(geom="text", x=5, y=Estimacion,
  ...
  annotate(geom="text", x=5, y=Estimacion,
           ...
           vjust=-0.4, family="font", fontface="italic", color="gray", size=4) +
  labs(title="",
       x="Año", y="Cantidad") +
